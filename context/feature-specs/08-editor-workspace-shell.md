Build the `/editor/[roomId]` workspace shell with server-side access checks. No canves. No canves logic yet.
...
Create `lib/project-access` with helpers for:
...
- existing `ProhectSidebar` in the left
...
- workspace layout renders with current project contact
